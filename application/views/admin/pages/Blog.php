    ...
    <div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-danger"  data-bs-dismiss="modal">Close</button>
                    <button type="button" id="addBtn"  class="btn btn-success">Add New Blog</button>
                </div>
            </div>
        </div>
    </div>
...
$(document).on('click','#editModalBtn',function(){
    ...
    $.ajax({
        type: 'GET',
        url:'<?=base_url('admin/blog/get')?>',
        data: {get_blog_data:0,id:dataId},
        cache: false,
        success: function(response) {
             var blog_data = JSON.parse(response);
            
            for(var i = 0; i < blog_data.length; i++) {
                var blog_data = blog_data[i];

                $('#update_blog_id').val(blog_data.id);
                $('#update_title').val(blog_data.title);
                $('#update_description').val(blog_data.description);
               
                $('#old_image').attr('src', blog_data.image);
