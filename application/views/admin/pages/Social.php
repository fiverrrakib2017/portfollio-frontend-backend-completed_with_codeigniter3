...
<script type="text/javascript">
	...
	$(document).on('click','#addBtn',function(){
		...
		if(title.length==0){
			...
			$.ajax({
				type: 'POST',
				url:'<?=base_url('admin/setting/social/add')?>',
				data: form_data,
				dataType: 'script',
				cache: false,
				contentType: false,
   ...
   var update_link= $('#update_link').val();
    ...
    if (update_title.length == 0) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?=base_url('admin/setting/social/update')?>', 
            data: form_data,
            dataType: 'script',
            cache: false,
            contentType: false,
    ...
    $(document).on('click','#deleteConfirmBtn',function(){
		var id=$(this).data('id');
		$.ajax({
            url: '<?=base_url('admin/setting/social/delete')?>', 
            type: 'POST',
...
$(document).on('click','#editModalBtn',function(){
    ...
    $.ajax({
        type: 'GET',
        url:'<?=base_url('admin/setting/social/get')?>',
