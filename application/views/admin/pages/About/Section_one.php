                                        ...
                                        <table id="template_table" class="table table-striped table-bordered" cellspacing="0"
                                            ...
                                            <?php foreach ($data as $item): ?>
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?php echo $item->complete_count; ?></td>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body ">
                    <form id="editForm" enctype="multipart/form-data">
                        ...
                        <div class="form-group mb-3 ">
                            <label for="title">Icon Text</label>
                            <input type="text" id="update_icon" class="form-control" placeholder="Enter Icon Text">
                        </div>
                        <div class="form-group mb-3 ">
                            <label for="title">Completed </label>
                            <input type="text" id="update_completed_count" class="form-control" placeholder="Enter Completed Count">
                        </div>
                    </form>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form id="editForm" enctype="multipart/form-data">         
                        <div class="form-group mb-3 ">
                            <label for="title">Title </label>
                            <input type="text" id="add_title" class="form-control" placeholder="Enter Title">
                        </div>
                        <div class="form-group mb-3 ">
                            <label for="title">Icon Text</label>
                            <input type="text" id="add_icon" class="form-control" placeholder="Enter Icon Text">
                        </div>
                        <div class="form-group mb-3 ">
                            <label for="title">Completed </label>
                            <input type="text" id="add_completed_count" class="form-control" placeholder="Enter Completed Count">
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
...
<script type="text/javascript">
	...
	$(document).on('click','#addBtn',function(){
		...
		var add_completed_count=$("#add_completed_count").val();


		/* Validation ruls  */
		 if(title.length==0){
			toastr.error('Title  Required');
		}else if(add_icon.length==0){
			toastr.error('Icon is Required');
		}else if(add_completed_count.length==0){
            toastr.error('Completed count is Required');
        }
			...
			form_data.append('add_icon', add_icon);
			form_data.append('add_completed_count', add_completed_count);
			form_data.append('add_data', add_data);
			/*Ajax calll Request Start */
			$.ajax({
				type: 'POST',
				url:'<?=base_url('admin/about/section/one/add')?>',
				data: form_data,
				dataType: 'script',
				cache: false,
				contentType: false,
    ...
    var update_completed_count= $("#update_completed_count").val();   
	var update_data=0;
    ...
    if (update_title.length == 0) {
        ...
        form_data.append('update_icon', update_icon);
        form_data.append('update_completed_count', update_completed_count);
        form_data.append('update_data', update_data);

        // AJAX call to update the student data
        $.ajax({
            type: 'POST',
            url: '<?=base_url('admin/about/section/one/update')?>', 
            data: form_data,
            dataType: 'script',
            cache: false,
            contentType: false,
    ...
    $(document).on('click','#deleteConfirmBtn',function(){
		var id=$(this).data('id');
		$.ajax({
            url: '<?=base_url('admin/about/section/one/delete')?>', 
            type: 'POST',
...
$(document).on('click','#editModalBtn',function(){
    ...
    $.ajax({
        type: 'GET',
        url:'<?=base_url('admin/about/section/one/get')?>',
        ...
        success: function(response) {
            ...
            for(var i = 0; i < jsonData.length; i++) {
                ...
                $("#update_completed_count").val(data.complete_count);             
